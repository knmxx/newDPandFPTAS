...
{
public:
	DataReader3(const char* filename);
	virtual ~DataReader3();
...
DataReader3::DataReader3(const char* filename)
{
	//ctor
...
DataReader3::~DataReader3()
{
	//dtor
}

vector<Job> DataReader3::next_case()
{
	if (current_case < data.size())
	{
		vector<Job> current_jobs = data[current_case];
		DataReader3::to_array(job_set, MAX_N, current_jobs);
...
bool DataReader3::has_next()
{
	return current_case < data.size();
}

bool DataReader3::to_array(Job* jobArray, int max_size, vector<Job> jobs)
{
	...
	PSUM = t;
	current_config.D = PSUM / 3;
	return true;
}

Configure DataReader3::get_current_configure()
{
	return configs[current_case - 1];
}

int DataReader3::get_current_case()
{
	return current_case;
}
